    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
              ->addColumn('action', function($query) {
                ...
                $deleteBtn = "<a href='".route('vendor.product-variant-item.destroy', $query->id)."' data-tableId='vendorproductvariantitem-table' class='btn btn-danger ms-2 delete-item'><i class='far fa-trash-alt'></i></a>";
              ...
              ->addColumn('default', function($query) {
                if($query->is_default == 1) {
                  return '<i class="badge bg-success rounded-pill">default</i>';
                }
              })
              ->addIndexColumn()
