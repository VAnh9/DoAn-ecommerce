...
class UserDashboardController extends Controller
{
  public function index()
  {
    ...
    $deliveredOrder = Order::where('user_id', Auth::user()->id)->where('order_status', 'delivered')->count();
    $review = ProductReview::where('user_id', Auth::user()->id)->count();
    $wishlist = Wishlist::where('user_id', Auth::user()->id)->count();
    return view('frontend.dashboard.dashboard', compact('totalOrder', 'deliveredOrder', 'pendingOrder', 'review', 'wishlist'));
  }
}
