          ...
          <div class="card">
            <div class="card-header">
              ...
              <div class="card-header-action">
                <a href="{{ route('admin.footer-social-links.index') }}" class="btn btn-secondary">Back</a>
              </div>
            </div>
            <div class="card-body">
              <form action="{{ route('admin.footer-social-links.update', $footerSocial->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="form-group" style="margin-bottom: 1rem">
                  <label>Icon</label>
                  <div>
                    <button name="icon" class="btn btn-light" data-selected-class="btn-danger"
                    data-unselected-class="btn-info" role="iconpicker" data-icon="{{ $footerSocial->icon }}"></button>
                  </div>
                  @if ($errors->has('icon'))
